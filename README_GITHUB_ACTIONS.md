...
GitHub Actions はリポジトリに 60 日間活動がないとスケジュール実行を自動で無効化します。
このワークフローでは `gautamkrishnar/keepalive-workflow` を使い、50 日ごとにダミーコミットを作成して無効化を防ぎます。
コミットメッセージには `[skip ci]` が含まれているため、プッシュによる追加実行は発生しません。
