...
export default function StatsCard({ events }: StatsCardProps) {
  ...
  const thisWeekEvents = events.filter(event => {
    ...
    const durationHours = (eventEnd.getTime() - eventStart.getTime()) / (1000 * 60 * 60)
    return eventDateString >= weekStartString && eventDateString <= weekEndString && durationHours < 24
  })
  ...
  const weeklyAverage = Math.round(thisWeekDuration / (1000 * 60 * 60) * 10) / 10
  ...
  return (
    <div className="bg-white/70 backdrop-blur-sm rounded-xl border border-gray-200 p-8 mb-8">
            ...
            <div className="space-y-1">
              <div className="flex items-baseline space-x-1">
                <span className={`text-3xl font-bold ${
                  stat.id === 'today-count' ? 'text-orange-600' :
                  stat.id === 'today-hours' ? 'text-blue-600' :
                  stat.id === 'weekly-events' ? 'text-purple-600' :
                  stat.id === 'weekly-average' ? 'text-green-600' :
                  'text-purple-600'
                }`}>
                  {stat.value}
                </span>
                <span className={`text-sm font-medium ${
                  stat.id === 'today-count' ? 'text-orange-500' :
                  stat.id === 'today-hours' ? 'text-blue-500' :
                  stat.id === 'weekly-events' ? 'text-purple-500' :
                  stat.id === 'weekly-average' ? 'text-green-500' :
                  'text-purple-500'
                }`}>
                  {stat.suffix}
                </span>
      ...
      <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">今日の予定一覧</h3>
